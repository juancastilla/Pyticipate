def shp2geojson(shapefile_path, src_epsg=None):
    ...
    for feat in layer:
        ...
        try:
            ...
            print('Failed to export JSON of a feature in shapefile: {}'.format(shapefile_path))
    return features
...
def build_shapefile_data(shapefile_path, src_epsg=None, ident_properties=[], metadata_properties=[], metadata_properties_aka=[]):
    ...
    import ftfy
    def get_prop(prop_val):
        if isinstance(prop_val, str):
            return ftfy.fix_text(prop_val) # Fix any encoding errors
        return str(prop_val)
    for feature in features:
