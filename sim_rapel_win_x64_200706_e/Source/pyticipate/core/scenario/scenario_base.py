...
class ChartDisplay():
    ...
    def __init__(self, x_label=None, y_label=None, decimalPlaces=None, title=None):
        """
        ...
        Args:
            ...
            decimalPlaces: (:obj:`int`, optional) The number of decimal places to display in the user-interface. None means display full precision. Default is None.
            title: (:obj:'str', optional) The title of the chart to display. Default is None. WARNING: Only partially implemented in front-end!
        """
...
class ParameterBase(ABC):
    ...
    def get_metadata(self):
        ...
        for k in self.__dict__.keys():
            if k != 'value':
                new_dict[k] = deepcopy(self.__dict__[k])
        return new_dict
...
class OutputTimeSeriesDef(OutputBaseDef):
    ...
    def __init__(self, id, name, grouping, display=None, description=None):
        """
        ...
        Args:
            ...
            description: (:obj:`str`, optional) Description HTML/text to display in chart info tool-tip. Defaults to None.
        """
...
class BoxNumericDatasetVal():
    ...
    def __init__(self, min, whiskerMin, q1, median, q3, whiskerMax, max, outliers=[]):
        """
        Used to pass a single box plot variables dataset to the `OutputBoxNumericDatasetVal`.

        Args:
            min: (numeric) The minimum value of the set (including outliers).
            whiskerMin: (numeric) The minimum value of the set (excluding outliers).
            ...
            whiskerMax: (numeric) The maximum value of the set (excluding outliers).
            max: (numeric) The maximum value of the set (including outliers).
            outliers: (List(numeric)) Outliers greater or less than 3/2 times of lower or upper quartiles.
        """
        self.min = min
        self.whiskerMin = whiskerMin
        self.q1 = q1
        self.median = median
        self.q3 = q3
        self.whiskerMax =whiskerMax
        self.max = max
...
class OutputBoxNumericDatasetVal(OutputBaseVal):
    ...
    def append_dataset(self, boxDatasets=[], name=None):
        ...
        for d in boxDatasets:
            values.append({'min':d.min,'whiskerMin':d.whiskerMin,'q1':d.q1,'median':d.median,'q3':d.q3,'whiskerMax':d.whiskerMax,'max':d.max,'outliers':d.outliers})
        self.value.append({'name':name,'values':values})
        return self
...
class LayerBaseDef(ParameterBase):
    ...
    def __init__(self, type, id, name, description, index, constant, show, display, group):
    ...
    def get_metadata(self):
        ...
        for k in self.__dict__.keys():
            if k != 'value':
                result[k] = deepcopy(self.__dict__[k])
...
class LayerRasterDef(LayerBaseDef):
    ...
    def __init__(self, id, name, description, geometry, index, constant, show, display=None, group=None):
        """
        A raster layer type.
        ...
        Args:
            ...
            group: (:obj:`str`, optional) A group name this layer belongs to. Default to None.
        """
        if display is None:
            display = RasterDisplay()
        super().__init__('Raster', id, name, description, index, constant, show, display, group)
        self.geometry = geometry

class LayerPointsDef(LayerBaseDef):

    def __init__(self, id, name, description, index, constant, show, display=None, group=None):
        """
        ...
        super().__init__('Points', id, name, description, index, constant, show, display, group)
...
class LayerLinesDef(LayerBaseDef):
    ...
    def __init__(self, id, name, description, index, constant, show, display=None, group=None):
        ...
        if display is None or display == []:
            ...
            display = [display]
        super().__init__('Lines', id, name, description, index, constant, show, display, group)
...
class LayerPolygonsDef(LayerBaseDef):
    ...
    def __init__(self, id, name, description, index, constant, show, display=None, group=None):
        ...
        if display is None or display == []:
            ...
            display = [display]
        super().__init__('Polygons', id, name, description, index, constant, show, display, group)
...
class LayerShapefileDef(LayerBaseDef):
    ...
    def __init__(self, id, name, description, index, constant, show, display=None, group=None):
        ...
        super().__init__('Shapefile', id, name, description, index, constant, show, display, group)
...
class LayerGeoTIFFDef(LayerBaseDef):
    ...
    def __init__(self, id, name, description, geometry, index, constant, show, display=None, group=None):
        ...
        self.geometry = geometry
        super().__init__('GeoTIFF', id, name, description, index, constant, show, display, group)

class LayerIconsDef(LayerBaseDef):

    def __init__(self, id, name, description, index, constant, show, group=None):
        """
        ...
        Args:
            ...
            group: (:obj:`str`, optional) A group name this layer belongs to. Default to None.
        """
        super().__init__('Icons', id, name, description, index, constant, show, None, group)
...
class LayerDataTimeSeriesWithIntervalsDef(LayerDataBaseDef):
    ...
    def __init__(self, display=None, description=None):
        ...
        Args:
            ...
            description: (:obj:`str`, optional) Description HTML/text to display in chart info tool-tip. Defaults to None.
        """
        super().__init__('TimeSeriesWithIntervals', {})
        if display is None:
            display = ChartDisplay()
        self.display = display.to_dict()
        if description is not None:
...
class LayerDataRadarDef(LayerDataBaseDef):
    ...
    def __init__(self, variables, description=None):
        """
...
class LayerDataHistogramDef(LayerDataBaseDef):
    ...
    def __init__(self, intervals, display=None, description=None):
        """
...
class LayerDataBoxDef(LayerDataBaseDef):
    ...
    def __init__(self, variables, display=None, description=None):
        """
...
class LayerDataGaugeDef(LayerDataBaseDef):
    ...
    def __init__(self, min, max, display=None, description=None):
        """
...
class ScenarioBase(ABC):
    ...
    def add_layer_data(self, layer_id, layer_data_defs, ident=None):
        """
        Add Layer Data/s to the model.

        Args:
            layer_id: (str) The Layer ID.
            layer_data_defs: (dict[str,:obj] or [:obj]) If is dictionary - keys are Layer Data ID names, values are Layer Data definitions.
            ident: (str) The feature identifier to use as a key for Layer Data lookup.
        """
        self._layer_data[layer_id] = dict()
        self._layer_data_values[layer_id] = dict()
        click_obj = {'ident':ident,'data':list()}
        data = list()
        if isinstance(layer_data_defs, dict):
            for data_id in layer_data_defs:
                ldd = layer_data_defs[data_id]
                d = dict()
                d['data_id'] = data_id
                d['type'] = ldd.type
                data.append(d)
                self._layer_data[layer_id][data_id] = layer_data_defs[data_id]
                self._layer_data_values[layer_id][data_id] = dict()
            click_obj['data'] = data
        elif isinstance(layer_data_defs, LayerDataBaseDef):
            data_id = 'DEFAULT_DATA_ID'
            d = dict()
            d['data_id'] = data_id
            d['type'] = layer_data_defs.type
            data.append(d)
            self._layer_data[layer_id][data_id] = layer_data_defs
            self._layer_data_values[layer_id][data_id] = dict()
            click_obj['data'] = data
        self._layers[layer_id].click = click_obj
    ...
    def set_layer_data(self, id, key, value, data_id=None):
        """
        ...
        Args:
            ...
            value: (:obj) The layer data values to set. Its type is dependent on the LayerDataDef originally used to define the Layer Data.
            data_id: (str) The Layer Data ID to if it exists.
        """
        if data_id is None:
            data_id = 'DEFAULT_DATA_ID'
        ...
        if isinstance(value, OutputBaseVal):
            ...
            self._layer_data_values[id][data_id][key] = value
    ...
    def reset_scenario(self):
        ...
        for id in self._layer_data:
            for data_id in self._layer_data[id]:
                self._layer_data[id][data_id].reset()
                self._layer_data_values[id][data_id] = dict()
        self._now = self._timestep.Subtract(self._start)
        self.reset()
    ...
    def run_scenario_time_step(self, num_timesteps=1):
        ...
        if self._multi_step:
            ...
            for id in self._layer_data:
                for data_id in self._layer_data[id]:
                    self._layer_data[id][data_id].set_value(self._layer_data_values[id][data_id], self._now)
        else:
            for i in range(num_timesteps):
                if self._now < self._end:
                    self._now = self._timestep.Add(self._now)
    ...
    def get_layer_data_metadata(self, layer_id, data_id):
        if layer_id in self._layer_data:
            if data_id in self._layer_data[layer_id]:
                return self._layer_data[layer_id][data_id].get_metadata()
        return None
    ...
    def get_layer_data(self, layer_id, data_id, type, ident):
        ident_type = ident['type']
        ident_value = ident['value']
        if ident_type == 'tuple':
            ident_value = (int(ident_value.split(',')[0][1:]),int(ident_value.split(',')[1][:-1]))
        data = self._layer_data[layer_id][data_id].get_value()
        if ident_value in data:
            return data[ident_value]
        else:
            return []

    def get_layer_data_valid_ids(self, layer_id, ident):
        ident_type = ident['type']
        ident_value = ident['value']
        if ident_type == 'tuple':
            ident_value = (int(ident_value.split(',')[0][1:]),int(ident_value.split(',')[1][:-1]))
        valid_data_ids = []
        for data_id in self._layer_data[layer_id].keys():
            data = self._layer_data[layer_id][data_id].get_value()
            if ident_value in data:
                valid_data_ids.append(data_id)
        return {'data_ids' : valid_data_ids}
