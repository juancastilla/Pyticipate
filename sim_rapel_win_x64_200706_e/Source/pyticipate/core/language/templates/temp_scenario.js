var scenario = (function() {
    ...
    function initialise_main() {
        ...
        document.addEventListener('jspanelclosed', function(event) {
            ...
            global_data_viewer.global_data_viewer_closed(event);
        });
    }
    ...
    var scenario = function() {
        ...
        function get_scenario_metadata(callback) {
            $.get('get_scenario_metadata/', function(response) {
                ...
                if (timestep_selector.length == 1) {
                    // Hide time step selector if only one option
                    $('#run-timestep-selector-div').hide();
                }
                var show_date_status = response['show_date_status'];
                if (!show_date_status) {
                    // Hide date controls
                    $('#status-div').hide();
                }
        ...
        function reset_scenario() {
            progress.show(function() { progress.update(100.0, '$-PROGRESS_RESET_TEXT-$'); } );
            ...
            ).always(function() { toggle_scenario_buttons(false, false, false); });
        }
        ...
        function populate_tile_options() {
            ...
            $('#map-tile-selector').multiselect({
                    templates: {
                        ...
                        button: '<button type="button" class="multiselect dropdown-toggle" data-toggle="dropdown" data-flip="false"><img class="map-tile-selector-img" src="/static/resources/img/globe-icon.png"></img></button>',
                    ...
                    buttonTitle: function(opt, sel) {
                        return '$-MAP_TILE_SELECTOR_DROPDOWN_TOOLTIP-$';
                    }
                });
    ...
    var map_layer_manager = function() {
        ...
        var map_click_manager = function() {
            ...
            var open_layer_data_dialog = function(e) {
                ...
                var data = info.data_ids;
                if (data.length === 1) {
                    var data_id = data[0].data_id;
                    var type = data[0].type;
                    layer_data_dialogs.spawn_layer_data_dialog(info.id, data_id, info.type, info.ident, info.title, latlng);
                }
                else {
                    var data_ids = [];
                    var types = [];
                    for (var i = 0; i < data.length; i++) {
                         data_ids.push(data[i].data_id);
                         types.push(data[i].type);
                    }
                    layer_data_dialogs.spawn_layer_data_selector(data_ids, function(data_id) {
                       var type = types[data_ids.indexOf(data_id)];
                        layer_data_dialogs.spawn_layer_data_dialog(info.id, data_id, info.type, info.ident, info.title, latlng);
                    });
                }
            };
        ...
        var map_layer_factory = function() {
            ...
            function create_map_layer(json_layer) {
                ...
                // Features
                map_layer.features = [];
                function on_feature_click(e, obj, feature_prop) {
                    var layer_id = obj.id;
                    var data = obj.click['data'];
                    var ident = {'type':'string','value':feature_prop};
                    var title = feature_prop + ' (' + obj.name + ')';
                    if (data.length === 1) {
                        var data_id = data[0].data_id;
                        var type = data[0].type;
                        layer_data_dialogs.spawn_layer_data_dialog(layer_id, data_id, type, ident, title, e.latlng);
                    }
                    else {
                        var data_ids = [];
                        var types = [];
                        ...
                        // Only show the layer data selector IF the specific feature contains that type of data
                        var args = {'layer_id': layer_id, 'ident': ident};
                        $.post('get_layer_data_valid_data_ids/', JSON.stringify(args), function(response) {
                            if (response === null) {
                                return;
                            }
                            data_ids = response['data_ids'];
                            if (data_ids.length > 0) {
                                layer_data_dialogs.spawn_layer_data_selector(data_ids, title, function(data_id) {
                                    var type = types[data_ids.indexOf(data_id)];
                                    layer_data_dialogs.spawn_layer_data_dialog(layer_id, data_id, type, ident, title, e.latlng);
                                });
                            }
                        }, 'json').fail(
                            function(e) { report_server_error('get_layer_data_valid_data_ids', e); }
                        );
                    }
                };
                if (type == 'Points') {
                        ...
                        var onEachFeature = function(feature, layer) {
                            ...
                            if (this_obj.click != null) {
                                ...
                                layer.on('click', function(e) {
                                    on_feature_click(e, this_obj, feature_prop);
                                });
                            }
        ...
        function initialise_layers() {
            $.get('get_layers/', function(response) {
                ...
                // Handle possible layer groupings
                var item_order = [];
                var groups = {};
                for (var i = 0; i < response.length; i++) {
                    ...
                    if (group_name != undefined) {
                        if (!(group_name in groups)) {
                            groups[group_name] = [];
                            item_order.push({'Group': group_name});
                        }
                        groups[group_name].push(response[i]);
                    }
                    else {
                        item_order.push({'Item': response[i]});
                    }
                }
                // NB: Are using drop-down control groups as a way of achieving the expand/collapse to show layer metadata
                // and so are manually changing group elements to insert our real Groups into the control
                var add_item = function(item) {
                    var map_layer = map_layer_factory.create_map_layer(item);
                ...
                for (var i = 0; i < item_order.length; i++) {
                    ...
                    if (item.hasOwnProperty('Group')) {
                        ...
                        var realGroupOpt = document.createElement('optgroup');
                        realGroupOpt.label = group_name;
                        realGroupOpt.classList.add('layer-data-group-item');
                        selector.add(realGroupOpt);    
                        for (var j = 0; j < groups[group_name].length; j++) {
                            add_item(groups[group_name][j]);
                        }
                    }
                    else {
                        add_item(item['Item']);
                ...
                $('#layer-selector').multiselect({
                    ...
                    buttonText: function(options, select) {
                        return '$-LAYER_SELECTOR_BUTTON_LABEL-$';
                    },
                    buttonTitle: function(opt, sel) {
                        return '$-LAYER_SELECTOR_TOOLTIP-$';
                    },
                ...
                // Redo Group elements (remove hyperlink, checkbox and caret)
                var group_items = $('#layer-selector-container .layer-data-group-item');
                for (var i = 0; i < group_items.length; i++) {
                    var ddi = group_items[i];
                    var a = ddi.childNodes[0];
                    var group_name = a.childNodes[0].childNodes[1].innerHTML;
                    ddi.removeChild(a);
                    var label = document.createElement("LABEL");
                    var b = document.createElement("B");
                    b.innerHTML = group_name;
                    label.appendChild(b);
                    ddi.appendChild(label);
                }
                get_layer_data();
            }, 'json').fail(
            ...
            $.post('get_groupings/', JSON.stringify(get_groupings_args), function(group_response) {
                ...
                function build_and_add(ios, group_element_id, tab_index, lookup) {
                    for (var k = 0; k < ios.length; k++) {
                        ...
                        // Attach output info tool-tip if description exists, otherwise hide info icon
                        var info_control_id = 'output-template-' + io_object.ELEMENT_TYPE + '-info-img-' + io_object.id;
                        if (io_object.description != undefined) {
                            attach_tooltip(info_control_id, io_object.description, 'left');
                        }
                        else {
                            $('#' + info_control_id).hide();
                        }
                    }
                }
    ...
    var layer_data_dialogs = function() {
        ...
        var layer_data_selector = function() {

            var layer_data_selector_template;

            function present_data_id_options(data_ids, title, callback) {
                if (typeof layer_data_selector_template === 'undefined') {
                    layer_data_selector_template = io_common.compile_templates([
                        'layer-data-dialog-template-selector'
                    ])['layer-data-dialog-template-selector'];
                }
                var uuidv4 = function() {
                ...
                var context = {id: control_id, names: data_ids, title: title};
                var html = layer_data_selector_template(context);
                var p = jsPanel.create({
                    id: 'layer-data-selector-' + control_id,
                    theme: 'primary',
                    contentSize: {
                        width: 500,
                        height: 180
                ...
                var ok_button = document.getElementById('layer-data-dialog-template-selector-ok-' + control_id);
                ok_button.onclick = function(e) {
                    var selector = document.getElementById('layer-data-dialog-template-selector-select-' + control_id);
                    var data_id = selector.options[selector.selectedIndex].text;
                    callback(data_id);
                    p.close();
                    
                };
                var cancel_button = document.getElementById('layer-data-dialog-template-selector-cancel-' + control_id);
            ...
            return {
                present_data_id_options: present_data_id_options
            }
        }();
        
        function spawn_layer_data_selector(data_ids, title, callback) {
            layer_data_selector.present_data_id_options(data_ids, title, callback);
        }
                    ...
                    data: [
                        {
                            ...
                            data_id: json_layer_data['data_id'],
        ...
        function spawn_layer_data_dialog(layer_id, data_id, type, ident, title, latlng) {
            if (typeof title === 'number') {
                title = title.toString();
            }
            document.body.style.cursor = 'progress';
            var args = {'layer_id': layer_id, 'data_id': data_id};
            $.post('get_layer_data_info/', JSON.stringify(args), function(response) {
                ...
                // Add chart tool-tip if description is provided, otherwise hide the info icon
                var info_control_id = 'layer-data-dialog-template-' + layer_data_view.ELEMENT_TYPE + '-info-img-' + control_id;
                if (response.hasOwnProperty('description')) {
                    ...
                    $('#' + info_control_id).tooltip(options);
                }
                else {
                    $('#' + info_control_id).hide();
                }
        ...
        function update_layer_data_dialog(dialog_view) {
            ...
            for (var i = 0; i < data.length; i++) {
                var d = data[i];
                var args = {'layer_id': d.id, 'data_id': d.data_id, 'type': d.type, 'ident': d.ident};
        ...
        return {
            spawn_layer_data_selector: spawn_layer_data_selector,
    ...
    var global_data_viewer = function() {

        var global_data_viewer_template;
        var global_data_viewer_chart_template;

        var global_data_views = [];

        function create_chart(global_data_view, response) {
            if (typeof global_data_viewer_chart_template === 'undefined') {
                var source = document.getElementById('global-data-chart-template').innerHTML;
                global_data_viewer_chart_template = Handlebars.compile(source);
            }

            var type = response['type'];
            
            if (type === 'TimeSeries') { type = 'timeseries'; }
            if (type === 'Radar') { type = 'radar'; }
            if (type === 'Histogram') { type = 'histogram'; }
            if (type === 'Doughnut') { type = 'doughnut'; }
            if (type === 'Box') { type = 'box'; }

            var control_id = global_data_view.control_id;
            var container = $('#global-data-viewer-data-div-'+control_id);
            var context = {id: control_id, type: type};
            var html = global_data_viewer_chart_template(context);
            container.html(html);

            // Add tool-tip if description data available
            var description = response['description'];
            var info_control_id = 'global-data-chart-template-' + type + '-info-img-' + global_data_view.control_id;
            ...
            if (type === 'timeseries') {
                ...
                if (datasets.length === 1) {
                    var ds = datasets[0]
                    var d = [];
                    for (var j = 0; j < ds.data.dates.length; j++) {
                        d.push({'date' : ds.data.dates[j], 'value' : ds.data.values[j]});
                    }
                    var cfg = charting.timeseries(d, display);
                }
                else {
                    var converted_datasets = [];
                    for (var i = 0; i < datasets.length; i++) {
                        ...
                        converted_datasets.push({ 'title' : ds.name, 'display' : display, 'data' : d });
                    }
                    var cfg = charting.multi_timeseries(converted_datasets);
                }
                var ctx = document.getElementById('global-data-chart-template-'+type+'-graph-'+control_id).getContext("2d");
                if (global_data_view.chart != null) { global_data_view.chart.destroy(); }
                global_data_view.chart = new Chart(ctx, cfg);
                charting.attach_click_event_handler('global-data-chart-template-'+type+'-graph-'+control_id, function(e) { global_data_view.chart.resetZoom(); });
            }
            else if (type === 'radar') {
                var labels = response['variables'];
                var datasets = response['data'];
                var converted_datasets = [];
                for (var i = 0; i < datasets.length; i++) {
                    var ds = datasets[i];
                    converted_datasets.push({ 'name' : ds.name, 'values' : ds.data });
                }
                var cfg = charting.radar(labels, converted_datasets);
                var ctx = document.getElementById('global-data-chart-template-'+type+'-graph-'+control_id).getContext("2d");
                if (global_data_view.chart != null) { global_data_view.chart.destroy(); }
                global_data_view.chart = new Chart(ctx, cfg);
            }
                ...
                var cfg = charting.histogram(labels, converted_datasets, display);
                var ctx = document.getElementById('global-data-chart-template-'+type+'-graph-'+control_id).getContext("2d");
                if (global_data_view.chart != null) { global_data_view.chart.destroy(); }
                global_data_view.chart = new Chart(ctx, cfg);
            }
                ...
                var cfg = charting.doughnut(labels, converted_datasets);
                var ctx = document.getElementById('global-data-chart-template-'+type+'-graph-'+control_id).getContext("2d");
                if (global_data_view.chart != null) { global_data_view.chart.destroy(); }
                global_data_view.chart = new Chart(ctx, cfg);
            }
                ...
                var cfg = charting.box(labels, converted_datasets, display);
                var ctx = document.getElementById('global-data-chart-template-'+type+'-graph-'+control_id).getContext("2d");
                if (global_data_view.chart != null) { global_data_view.chart.destroy(); }
                global_data_view.chart = new Chart(ctx, cfg);
            }
        ...
        function update_global_data_chart(global_data_view) {
            var strategy_ids = [];
            var data_id = global_data_view.data_id;
            for (var i = 0; i < global_data_view.strategy_ids.length; i++) {
                var strat_info = global_data_view.strategy_ids[i];
                if (strat_info.selected) {
                    strategy_ids.push(strat_info.id);
                }
            }
            if (strategy_ids.length === 0 || data_id === '') {
                if (global_data_view.chart != null) {
                    global_data_view.chart.destroy();
                    global_data_view.chart = null;
                }
                return;
            }
            var global_data_args = { 'strategy_ids' : strategy_ids, 'data_id' : data_id };
            $.post('get_global_data/', JSON.stringify(global_data_args), function(response) {
                create_chart(global_data_view, response);
            }, 'json').fail(
                function(e) { report_server_error('request global data', e); }
            );
        }

        function compile_global_data_viewer_template() {
            var source = document.getElementById('global-data-viewer-template').innerHTML;
            return Handlebars.compile(source);
        }

        function build_tree_json(data, json) {
            for (var i = 0; i < data.length; i++) {
                var id = data[i].id;
                ...
                json.push(jd);
                build_tree_json(children, jd.children);
            }
            return json;
        }

        function open_viewer() {
            if (typeof global_data_viewer_template === 'undefined') {
                global_data_viewer_template = compile_global_data_viewer_template();
            }
            $.get('get_global_data_info/', function(response) {
                ...
                global_data_view.update_strategy = function(strategy_name, selected) {
                    for (var i = 0; i < this.strategy_ids.length; i++) {
                        if (this.strategy_ids[i].name == strategy_name) {
                            this.strategy_ids[i].selected = selected;
                        }
                    }
                    this.update_chart();
                };
                global_data_view.update_data = function(data_id) {
                    this.data_id = data_id;
                    this.update_chart();
                };
                global_data_view.destroy = function() {
                    if (this.chart != null) {
                        this.chart.destroy();
                    }
                };
                global_data_view.update_chart = function() {
                    update_global_data_chart(this);
                };
                
                var template_options = [];
                for (var i = 0; i < strategies.length; i++) {
                    var id = strategies[i].id;
                    var name = strategies[i].name;
                    var ticked = (i === 0);
                    template_options.push({ 'name' : name, 'ticked' : ticked });
                    global_data_view.strategy_ids.push( { 'name' : name, 'id': id, 'selected' : ticked } );
                }
                var context = {id: control_id, options: template_options};
                var html = global_data_viewer_template(context);
                var title = '$-GLOBAL_DATA_VIEWER_NAME-$';
                var p = jsPanel.create({
                    id: 'global-data-viewer-' + control_id,
                    theme: 'primary',
                    contentSize: {
                        width: 1200,
                        height: 600
                ...
                p.headertitle.title = title;
                // Initialise Strategy control
                // Hack from GitHub issue comment to get bootstrap-multiselect working in BS4-beta
                $('#global-data-viewer-strategy-options-'+control_id).multiselect({
                    templates: {
                        li: '<li><a class="dropdown-item"><label class="m-0 pl-2 pr-0"></label></a></li>',
                        ul: ' <ul class="multiselect-container dropdown-menu p-1 m-0 global-data-viewer-dropdown-ul"></ul>',
                        button: '<button type="button" class="multiselect dropdown-toggle" data-toggle="dropdown" data-flip="false" data-size="5"><span class="multiselect-selected-text"></span> <b class="caret"></b></button>',
                        filter: '<li class="multiselect-item filter"><div class="input-group m-0"><input class="form-control multiselect-search" type="text"></div></li>',
                        filterClearBtn: '<span class="input-group-btn"><button class="btn btn-secondary multiselect-clear-filter" type="button"><i class="fas fa-minus-circle"></i></button></span>'
                    },
                    maxHeight: 185,
                    ...
                    buttonClass: 'btn btn-secondary',
                    onChange: function(opt, checked, sel) {
                        global_data_view.update_strategy($(opt).val(), checked);
                    }
                });
                // Initialise Data control
                var tree_json = [];
                build_tree_json(data, tree_json);
                $('#global-data-viewer-data-tree-' + control_id).on('changed.jstree', function (e, data) {
                    if (data.selected.length > 0) {
                        var selected_data_id = data.selected[0];
                        global_data_view.update_data(selected_data_id);
                    }
                }).jstree(
                    {
                        'core' : {
                ...
                register_global_data_view(global_data_view);
                // Hide and show left menu code
                $('#global-data-viewer-hide-img-' + control_id).click(function(e) {
                    $('#global-data-menu-div-' + control_id).hide();
                    $('#global-data-viewer-hide-div-' + control_id).hide();
                    $('#global-data-viewer-show-div-' + control_id).show();
                    $('#global-data-viewer-data-div-' + control_id).css('padding-left', '40px');
                    global_data_view.update_chart();
                });
                $('#global-data-viewer-show-img-' + control_id).click(function(e) {
                    $('#global-data-menu-div-' + control_id).show();
                    $('#global-data-viewer-show-div-' + control_id).hide();
                    $('#global-data-viewer-hide-div-' + control_id).show();
                    $('#global-data-viewer-data-div-' + control_id).css('padding-left', '500px');
                    global_data_view.update_chart();
                });
            }, 'json').fail(
                function(e) { report_server_error('request global data info', e); }
            );
        }

        function register_global_data_view(global_data_view) {
            global_data_views.push(global_data_view);
        }

        function remove_global_data_view(global_data_view) {
            var index = -1;
            for (var i = 0; i < global_data_views.length; i++) {
                if (global_data_view === global_data_views[i]) {
                    index = i;
                    break;
                }
            }
            if (index >= 0) {
                global_data_view.destroy();
                global_data_views.splice(index, 1);
            }
        }

        function global_data_viewer_closed(event) {
            for (var i = 0; i < global_data_views.length; i++) {
                var view = global_data_views[i];
                if ('global-data-viewer-' + view.control_id == event.detail) {
                    remove_global_data_view(view);
                }
            }
        }
        ...
        return {
            ...
            global_data_viewer_closed: global_data_viewer_closed
        }
    }();
    ...
    var charting = function() {
        ...
        function generate_timeseries_config(labels, datasets, display_legend, x_label, y_label, decimal_places, title) {
        ...
        function timeseries(data, display) {
            ...
            return generate_timeseries_config(labels, datasets, false, display['x-label'], display['y-label'], display['decimal-places'], display['title']);
        }
        ...
        function radar(labels, data) {
            ...
            var datasets = [];
            for (var i = 0; i < data.length; i++) {
                datasets.push(create_radar_dataset(data[i]['name'], get_rgba(colors[i], 0.8/*0.2 for fill*/), data[i]['values']));
            }
        ...
        function generate_box_config(labels, datasets, x_label, y_label, decimal_places, title) {
            var legend = false;
            if (datasets.length > 1) { legend = { position: 'top' } }
            var display_x_label = true;
            if (x_label === null) { display_x_label = false; }
        ...
        function box(labels, data, display) {
            ...
            return generate_box_config(labels, datasets, display['x-label'], display['y-label'], display['decimal-places'], display['title']);
        }
    ...
    return {
        ...
        open_global_data_viewer: global_data_viewer.open_viewer
    }
}());